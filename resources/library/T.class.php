...
class T {
	...
	public static function select(Attribute $attribute, array $options,
			$optionsSimpleArray=false, $alternativeName=false, $js=false) {
		$name = $alternativeName ? "name=\"model[$alternativeName]\"" :
		"name=\"model[{$attribute->getName()}]\"";
		$nullable = $attribute->getNullable();
		$value = "value=\"{$attribute->getValue()}\"";
	...
	public static function button($type, $text) {
		...
		return "<button $typeText name=\"$type\" value=\"$type\"
		class=\"button text $type\">$text</button>";
	}

	public static function href($controller="welcome", $action="index",
			$array=array()) {
		$string = "";
	...
	public static function iconButton($icon, $text, $controller, $action="index",
			$array=array()) {
		$type = $text == false ? "notext" : "text";
		return "<a href=\"" . self::href($controller, $action, $array) . "\"
		class=\"button $type $icon\">$text</a>";
	}
}
?>