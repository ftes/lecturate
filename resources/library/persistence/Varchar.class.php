	...
	public function generateErrors() {
		...
		if (is_null($this->value)) {
			...
			if (! is_string($this->value)) {
				...
				if ($this->maxLength && strlen($this->value) >
						$this->maxLength) array_push($this->errors,
								"Zu lang (max. {$this->maxLength})");
				if ($this->minLength && strlen($this->value) <
						$this->minLength) array_push($this->errors,
								"Zu kurz (min. {$this->minLength})");
			}
		}
	}
